...
| Gutters (`gutter`) — a 4-ft seam section at the arena and hangar gutter hotspots; the rest of both runs is code of the same section | W-7-TYP (gutter-seams sheet; the concrete-wall sheet carries the same number, so the panel renders this one as `W-7-TYP-GUTTER`) | Standing seam, R-panel, SPF | MODELLED+SPLICED | Every gutter size, the 1" lap and its rivets, coat thicknesses (§5 #26); the sheet's 2" band and whole-interior topcoat are VERIFIED |
| Downspout inlets (`gutterinlet`) — a 4-ft inlet section on a downspout at the arena and hangar inlet hotspots; the other outlets code | D-8-TYP | Standing seam, R-panel, SPF | MODELLED+SPLICED | Outlet size and flange, the strainer basket (no sheet shows one), 12" of drop, coat thicknesses (§5 #26); the 3" out / 3" in / 1" further extents are VERIFIED |
...
| Expansion joint (`ej`) — hospital: a 4-ft section at the hotspot; the rest of the run is a code strip of the same section, flanges flat on the roof as drawn | A-3-TYP | Gravel BUR, mod-bit, TPO, concrete | MODELLED+SPLICED | Joint, flange, bellow and fastener sizes; no 3D render (§5 #16) |
| Silo walls (`silowall`) — a 4-ft ring of the hotspot bin's wall; the other five bins and the rest of this one stay code, coated the same way | W-7-TYP (concrete-wall sheet) + Plate C for the crack repair | Concrete | MODELLED+SPLICED | Everything about extents: full-height coats on a curved wall, joint spacing, crack repair, every size (§5 #17) |
| Gallery supports (`support`) — silo: the hotspot post; the other five draw the same wrap in code | P-5-C, P-7-C | Concrete | MODELLED+SPLICED | Post, gap, bead, backer-rod and old-mastic sizes (§5 #23); the wrap extents are from the sheet |
...
**Totals:** 27 MODELLED+SPLICED · 0 MODELLED · 1 CODE-DRAWN · 0 NOT STARTED.
...
| 2 | Remaining detail models | IN PROGRESS | 1 CODE-DRAWN row left in §1 (Solar Post supports). 25 Sept 2026: the gutter pair (W-7-TYP, D-8-TYP) as the third splice pattern, eave-hung, and the silo wall (W-7-TYP concrete sheet) as the fourth, a ring on a curved wall — every extent of it ASSUMED pending RMI (§5 #17) |
...
- [x] Gravel BUR prep stage: white speckles on the bare felts — the building box's top face lay level with the field plane and z-fought it; it now sits 1/4" under the roof on every flat building
