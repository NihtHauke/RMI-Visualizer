...
| # | Feature | Status | Notes |
|---|---|---|---|
| 0 | Desktop installer (Electron, Windows) | BUILT (v0.2.0) | `npm run dist` → `dist/RMI Roof Visualizer Setup 0.2.0.exe`; RMI crest as the app/taskbar icon (`build/icon.ico`, `build/icon.png`) and the RMI logo (`assets/rmi-logo.png`) in the header, on the PDF cover and in the PDF page header; runs offline, vendored libs; bundles `drawings/` (render it first). `RMI_SELFTEST=<png>` opens the big-box drain with the Drawing panel, prints its state and saves a screenshot; `RMI_SELFTEST_PDF=<pdf>` exports the big-box presentation with the sample photos to that path (the build checks). Window title shows the version from package.json at runtime (`app.getVersion()`), e.g. "RMI Roof Visualizer 0.2.0". Mac only if needed. Rollout to reps not yet recorded |
