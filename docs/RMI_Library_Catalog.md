...
| Vents (`vent`) | Plate D · no 3D render · SPF → SPF-16-TYP · concrete → **Bin vent**, CS-12-CON · TPO / mod-bit / gravel (EagleView prospect) → **Vent / duct curb**, CS-1-TYP note 7 | `bin-vent-CS-1-TYP.glb` on the silo | **ASSUMED** on metal: no dedicated vent drawing for metal roofs in the library | The whole metal assembly; wrap height on SPF; the concrete case treated as a curb per CS-12-CON |
