...
| Silo walls (`silowall`) | W-7-TYP, concrete and CMU walls (the CONCRETE WALL – TYPICAL sheet under Walls; the gutter sheet carries the same number) · Plate C for the crack repair · no 3D render | `silo-wall-W-7-TYP.glb`: a 4-ft ring of the hotspot bin's wall at the hotspot height, spliced into the code-drawn bin (`scripts/build_silo_wall_W-7-TYP.py`); the rest of that bin and the other five stay code, coated the same way | **VERIFIED** from the sheet: (E) concrete wall; check all (E) panel joint sealant and replace damaged, aged or cracked sealant; clean, repair, prepare and prime for each type of surface; RMI-Flex vapor barrier base coat (rate may increase with the wall finish); RMI-Thane / RMI-White over it; all concrete wall panels and finishes (note 7); adhesion test and moisture scan first (note 8). From Plate C: spalls and cracks repaired to concrete standards (patch, epoxy injection, joints), patch to match the concrete, Flex at every repair | **Everything about extents**, pending RMI's answer (§5 #17): that a flat-wall sheet and a roof plate carry over to a curved slipformed bin sprayed vertically; full-height coats; eight 1" control joints per bin with sealant, a 3/4" lift line, three hairline cracks routed and patched 1-1/2" wide, the 1" wall shell, coat thicknesses |
...
| Silo walls: code cylinders with full-height coat overlays, "per W-7-TYP" | The hotspot bin carries a modelled ring to the concrete-wall sheet (sealant check and replacement, clean / repair / prime, Flex base coat, Thane) with the crack repair from Plate C (patch to match, Flex at every repair). Nothing about extents is on any document — it all stays ASSUMED and the detail text says so (§5 #17) | W-7-TYP (wall sheet), Plate C, 25 Sept 2026 |
...
17. **Vertical application on silo walls.** W-7-TYP covers concrete and CMU walls, but sequence and coverage on
    a curved silo wall come from the Longview grain terminal project rather than a drawing. Modelled 25 Sept 2026 as a
    4-ft ring of one bin's wall (`scripts/build_silo_wall_W-7-TYP.py`), with everything about extents ASSUMED and said so
    in the tool: the whole wall coated full height, primer then Flex then Thane sprayed vertically; eight vertical control
    joints per bin (1" wide, 1/2" deep) with the (E) sealant raked out and replaced per the sheet; a 3/4" construction
    (lift) line at mid-band; hairline cracks routed and patched 1-1/2" wide per Plate C, Flex over the repairs. Open: does
    RMI coat bin walls full height or only a band; is a jump-form / slipform bin's construction line treated as a joint;
    is the Plate C crack rule (patch, epoxy injection) what RMI does on a wall; a typical joint spacing; and the vertical
    spray rate (Flex "may increase depending on wall finish"). Constants at the top of the build script.
